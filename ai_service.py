...
Ensure all strings are properly escaped. No trailing commas. Valid JSON only.

Return this exact structure:
{{"currentGaps": ["gap1", "gap2"], "skillsToAcquire": ["skill1", "skill2"], "actionPlan": [{{"phase": "Phase 1", "duration": "3 months", "actions": ["action1", "action2"]}}], "resources": ["resource1", "resource2"], "milestones": ["milestone1", "milestone2"]}}"""
    ...
    try:
        ...
        # Try parsing with better error reporting
        try:
            data = json.loads(text)
        except json.JSONDecodeError as json_err:
            # Log the problematic JSON for debugging
            error_msg = f"JSON parse error at position {json_err.pos}: {json_err.msg}\n"
            error_msg += f"First 500 chars of cleaned response: {text[:500]}\n"
            error_msg += f"Last 200 chars of cleaned response: {text[-200:]}\n"
            error_msg += f"Full raw response length: {len(raw_text)} chars"
            
            # Try to extract JSON using regex as fallback
            json_match = re.search(r'\{.*\}', text, re.DOTALL)
            if json_match:
                try:
                    fallback_text = json_match.group(0)
                    # Fix common issues
                    fallback_text = re.sub(r',(\s*[}\]])', r'\1', fallback_text)
                    data = json.loads(fallback_text)
                except:
                    raise ValueError(f"Failed to parse JSON response. {error_msg}")
            else:
                raise ValueError(f"Failed to parse JSON response. {error_msg}")
        
        return {
            "currentGaps": data.get("currentGaps", [])[:6],
            "skillsToAcquire": data.get("skillsToAcquire", [])[:8],
            "actionPlan": data.get("actionPlan", [])[:4],
