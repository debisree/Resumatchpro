...
def create_app():
    ...
    # Session configuration - sessions expire when browser closes (non-permanent)
    # Set a default lifetime but sessions will be non-permanent (expire on browser close)
    app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(days=1)  # Default, but sessions are non-permanent
    app.config['SESSION_COOKIE_HTTPONLY'] = True
    app.config['SESSION_COOKIE_SAMESITE'] = 'Lax'
    
    # Store server start time to invalidate sessions after restart
    app.config['SERVER_START_TIME'] = time.time()
    
    # Only apply connection pooling options for PostgreSQL, not SQLite
    ...
    # Validate session on each request - clear invalid sessions
    @app.before_request
    def validate_session():
        if 'user_id' in session:
            # Check if session was created before server start (invalid after restart)
            session_start_time = session.get('_created_at', 0)
            if session_start_time < app.config['SERVER_START_TIME']:
...
@app.route('/')
def index():
    # Show landing/login page - if logged in, user can still access home via direct URL
    # Sessions will expire when browser closes (non-permanent)
...
def register():
    if request.method == 'POST':
        ...
        # Set session as non-permanent (expires when browser closes)
        session.permanent = False
        session['user_id'] = user.id
        session['_created_at'] = time.time()  # Track when session was created
...
def delete_resume(resume_id):
    ...
    try:
        # Delete related career roadmaps (analyses and job_matches are cascade deleted)
        CareerRoadmap.query.filter_by(resume_id=resume_id).delete()
        
        # Delete the resume (cascade will handle analyses and job_matches)
